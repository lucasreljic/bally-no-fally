    ...
    try:
        while True:
            ...
            for d in detections:
                ...
                # On-screen overlays
                cv2.putText(
                    frame, f"x={x:+.3f} m  y={y:+.3f} m  z={z:+.3f} m",
                    (center[0] + 6, center[1] + 16),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (240, 240, 240), 1
                )
                cv2.putText(
                    frame, f"dist={dist_m:.3f} m  yaw={yaw:+.1f}  pitch={pitch:+.1f}  roll={roll:+.1f}",
                    (center[0] + 6, center[1] + 36),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200, 200, 200), 1
                )

                # Log line (easy to parse)
                print(
                    f"tag_id={d.tag_id}  "
