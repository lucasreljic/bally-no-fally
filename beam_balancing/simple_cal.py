...
class SimpleAutoCalibrator:
    ...
    def find_limits_automatically(self):
        ...
        if len(positions) >= 2:
            ...
            print(
                f"[LIMITS] Range: {self.position_min:.4f}m to {self.position_max:.4f}m"
            )
        else:
    ...
    def save_config(self):
        ...
        config = {
            ...
            "ball_detection": {
                "lower_hsv": [float(x) for x in self.lower_hsv]
                if self.lower_hsv
                else None,
                "upper_hsv": [float(x) for x in self.upper_hsv]
                if self.upper_hsv
                else None,
            },
            "calibration": {
                ...
                "position_min_m": float(self.position_min)
                if self.position_min
                else None,
                "position_max_m": float(self.position_max)
    ...
    def draw_overlay(self, frame):
        ...
        if self.hsv_samples:
            cv2.putText(
                overlay,
                f"Color samples: {len(self.hsv_samples)}",
                (10, 90),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.5,
                (0, 255, 0),
        ...
        if self.position_min is not None and self.position_max is not None:
            cv2.putText(
                overlay,
                f"Limits: {self.position_min:.4f}m to {self.position_max:.4f}m",
