...
class BasicPIDController:
    ...
    def create_gui(self):
        ...
        self.kp_label = ttk.Label(
            self.root, text=f"Kp: {self.Kp:.1f}", font=("Arial", 11)
        )
        self.kp_label.pack()
        ...
        ki_slider = ttk.Scale(
            ...
            from_=0,
            to=10,
            variable=self.ki_var,
            orient=tk.HORIZONTAL,
            length=500,
        )
        ki_slider.pack(pady=5)
        self.ki_label = ttk.Label(
            self.root, text=f"Ki: {self.Ki:.1f}", font=("Arial", 11)
        )
        self.ki_label.pack()
        ...
        self.kd_label = ttk.Label(
            self.root, text=f"Kd: {self.Kd:.1f}", font=("Arial", 11)
        )
        self.kd_label.pack()
        ...
        self.setpoint_label = ttk.Label(
            self.root, text=f"Setpoint: {self.setpoint:.3f}m", font=("Arial", 11)
        )
        ...
        ttk.Button(
            button_frame, text="Reset Integral", command=self.reset_integral
        ).pack(side=tk.LEFT, padx=5)
        ttk.Button(button_frame, text="Plot Results", command=self.plot_results).pack(
            side=tk.LEFT, padx=5
        )
        ttk.Button(button_frame, text="Stop", command=self.stop).pack(
            side=tk.LEFT, padx=5
        )
    ...
    def plot_results(self):
        ...
        ax1.set_title(
            f"Basic PID Control (Kp={self.Kp:.1f}, Ki={self.Ki:.1f}, Kd={self.Kd:.1f})"
        )
        ax1.legend()
